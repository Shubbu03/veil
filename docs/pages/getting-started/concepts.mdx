...
```typescript
import { buildMerkleTree } from "@veil/sdk";
...
When creating a schedule, funds move from `available` → `reserved`. When schedule completes or cancels, funds return to `available`.
